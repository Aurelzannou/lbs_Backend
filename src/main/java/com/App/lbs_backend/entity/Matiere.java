package com.App.lbs_backend.entity;

import com.App.lbs_backend.core.AuditableEntity;
import jakarta.persistence.*;
import java.util.UUID;

import com.App.lbs_backend.core.Timestamps;

@Entity
@Table(name = "lbs_matiere", schema = "lbs")
@AttributeOverrides({
    @AttributeOverride(name = "modifierLe", column = @Column(name = "lbs_mati_modifier_le")),
    @AttributeOverride(name = "modifierPar", column = @Column(name = "lbs_mati_modifier_par", length = 100))
})
public class Matiere extends AuditableEntity implements Timestamps {

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long id;

    @Column(name = "lbs_mati_uuid", length = 50, unique = true, nullable = false)
    private String uuid;

    @Column(name = "lbs_mati_code", length = 20)
    private String code;

    @Column(name = "lbs_mati_libelle", length = 150)
    private String libelle;

    @Column(name = "lbs_mati_actif")
    private Boolean actif;

    /** Marque la matière "Conduite" — sa moyenne n'est jamais divisée par 2 (une seule valeur
        suffit) et une suggestion automatique (18 - nombre d'absences) est proposée à l'admin.
        Déduit automatiquement du code / libellé (« CONDUITE ») : aucun bouton à cocher. */
    @Column(name = "lbs_mati_est_conduite")
    private Boolean estConduite;

    @PrePersist
    @PreUpdate
    public void avantEcriture() {
        if (this.uuid == null) this.uuid = UUID.randomUUID().toString();
        if (this.actif == null) this.actif = true;
        this.estConduite = deduireEstConduite();
    }

    /** Une matière est "Conduite" si son code ou son libellé contient le mot « conduite ». */
    private boolean deduireEstConduite() {
        String c = code != null ? code.trim().toLowerCase() : "";
        String l = libelle != null ? libelle.trim().toLowerCase() : "";
        return c.contains("conduite") || l.contains("conduite");
    }

    public Matiere() {}

    public Long getId() { return id; }
    public void setId(Long id) { this.id = id; }

    public String getUuid() { return uuid; }
    public void setUuid(String uuid) { this.uuid = uuid; }

    public String getCode() { return code; }
    public void setCode(String code) { this.code = code; }

    public String getLibelle() { return libelle; }
    public void setLibelle(String libelle) { this.libelle = libelle; }

    public Boolean getActif() { return actif; }
    public void setActif(Boolean actif) { this.actif = actif; }

    public Boolean getEstConduite() { return estConduite; }
    public void setEstConduite(Boolean estConduite) { this.estConduite = estConduite; }
}
